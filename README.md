...
Сервер ждёт такие поля:

- `name` — имя, обязательное
- `phone` — телефон, обязательное
- `pet` — питомец, необязательное
- `message` — сообщение, необязательное
- `consent` — чекбокс согласия, обязательное
- `website` — honeypot, должен быть пустым
- `startedAt` — timestamp, обязательное для антиспама
- `subject` — тема заявки, необязательное, по умолчанию `Заявка с сайта ИВВС`

Если `message` или `pet` пустые, они всё равно попадут в письмо как `—`.
