...
Минимальное приложение для Vercel: HTML-форма отправляет данные на серверный endpoint, а сервер пересылает их на почту через SMTP.

Подходит для сценария, где HTML-фронты живут на других доменах: они отправляют `POST` на этот Vercel endpoint, а сервер уже шлёт письмо. Секреты остаются только на сервере.
